<script lang="ts">
	...
	function getActiveBackgroundLabel() {
		...
		if (store.backgroundType === "gradient") {
			return (
				GRADIENT_PRESETS.find((gradient) => gradient.value === store.backgroundValue)
...
<div class="flex flex-col gap-4 animate-in fade-in duration-300">
				...
				{#each WALLPAPERS as wallpaper}
					...
					>
						<img
							...
							draggable="false"
						/>
						<div class="absolute inset-x-0 bottom-0 bg-linear-to-t from-black/65 to-transparent px-2 pb-2 pt-4 text-[10px] font-medium text-white/90">
							{wallpaper.label}
						</div>
					</button>
				{/each}
	...
	{:else if store.backgroundType === "color"}
		<section class="rounded-3xl border border-border/70 bg-card/80 p-4 shadow-sm">
			...
			<div class="mt-4 rounded-2xl border border-border/70 bg-background/70 p-3">
	...
	{:else if store.backgroundType === "gradient"}
		<section class="rounded-3xl border border-border/70 bg-card/80 p-4 shadow-sm">
			<div class="mb-3 flex items-center gap-2">
				...
				<InspectorHint content="These presets render in the preview using their selected gradient values." />
			</div>
				...
				{#each GRADIENT_PRESETS as gradient}
					...
					>
						<div class="flex h-full items-end">
							<span class="rounded-full bg-black/35 px-2 py-1 text-[11px] font-semibold text-white shadow-sm backdrop-blur-sm">
								{gradient.label}
							</span>
						</div>
					</button>
	...
	{:else}
		<section class="rounded-3xl border border-border/70 bg-card/80 p-4 shadow-sm">
			<div class="flex items-center justify-between gap-3">
				<div class="flex items-center gap-2">
					<h4 class="text-sm font-semibold text-foreground">Image Background</h4>
					<InspectorHint content="Imported images are fit to cover the full canvas in the editor preview." />
				</div>
				<button
					type="button"
			...
			{:else}
				<div class="mt-3 flex h-30 items-center justify-center rounded-2xl border border-dashed border-border/70 bg-background/60 text-xs text-muted-foreground">
					Choose an image
				</div>
			{/if}
		</section>
